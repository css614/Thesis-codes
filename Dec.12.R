...
judgment<-function(x1,y1,x2,y2)
{
  ...
  if(length(which((x2-x1)*(corner_y-y1)-(y2-y1)*(corner_x -x1)<=0))!=4&length(which((x2-x1)*(corner_y-y1)-(y2-y1)*(corner_x -x1)>=0))!=4)
  {
...
entrance_interval_sequence<-c(60)

entrance_interval<-80

for(entrance_interval in entrance_interval_sequence)
{
  while(sum(success[,entrance_interval])<100) ## 成功运行的次数，相当于monte-carlo，最后以平均值作为最后的估计值
  {
    customer.numeber<-1
    ...
    customer.all<-shopping_list("gamma",15,1,1)
    ...
    for(i in 1:14399)
    {
      ...
      if (floor((i-1)/entrance_interval)>=0&((i-1)/entrance_interval)==floor((i-1)/entrance_interval))
      {
        ...
        {
          if(time_all[i,(2*(customer.numeber-2)-1)]==30&time_all[i,2*(customer.numeber-2)]==25) ## 上一个顾客已经被堵在门口不能动了
          {
            freezing<-"stuck"
            print("stuck")
            break
  ...
  ## if we have 100 times "no freezing", we have recorded the spending rate for all success[1,entrance_interval] times
  ## so here we need use average_total_spending_rate to be divided by success[1,entrance_interval]
  average_total_spending_rate[1:14400,entrance_interval]<-average_total_spending_rate[1:14400,entrance_interval]/success[1,entrance_interval]
...
plot(c(0.4,0.45,0.52),c(0.6,0.55,0.48)
     ,xlab = "success proportion",ylab = "freezing proportion",xlim=c(0,1),ylim=c(0,1)
     ,pch=19,col=c("red","blue","black"))
legend(0,1,c("gamma(3,0.2)","gamma(15,1)","gamma(150,10)"),col = c("red","blue","black")
       ,text.col = c("red","blue","black"),bty = "n",cex = 0.8,pch = 19)
text(c(0.38, 0.55, 0.6), 
       c(0.65, 0.55, 0.45),
       c("(0.4,0.6)", "(0.45,0.55)", "(0.52,0.48)"),
...
a4_15<-average_shopping_time
a4_150<-average_shopping_time
a4_3<-average_shopping_time
mean(a4_15[1,entrance_interval])
mean(a4_150[1,entrance_interval])
mean(a4_3[1,entrance_interval])
mean(average_shopping_time[1:10,entrance_interval_sequence])


a5_15<-customer_number_in_store_save
a5_150<-customer_number_in_store_save
a5_3<-customer_number_in_store_save

a5_3<-read.csv("4hours.csv",header = TRUE)
plot(a5_3[1:14399,2],xlab = "time",ylab = "number"
     ,main = "#of customers in the store ",col="black",ylim = c(0,50),type = "l")

a5_3<-a5_3[1:14399,c(1,2,4,5)]
write.csv(a5_3,"a5_3.csv")

a5_150<-a5_150[1:14399,c(1,2,8)]
a5_150_80<-a5_150[1:14399,c(4:6)]
write.csv(a5_150,"a5_150.csv")

a5_15<-read.csv("a5_15.csv",header = TRUE)
write.csv(a5_15,"a5_15.csv")

a5<-cbind(a5_150[1:14399,3],a5_15[1:14399,2],a5_3[1:14399,3])
write.csv(a5,"a5_data.csv")

a<-read.csv("a5_data.csv",header = TRUE)

plot(a[1:14399,2],xlab = "time",ylab = "number"
     ,main = "#of customers in the store ",col="black",ylim = c(0,50),type = "l")
lines(a[1:14399,3],col="blue")
lines(a[1:14399,4],col="red")

###################################
smoothed <- smooth.spline(a[,2], spar = 0.6)$y
library(signal)
smoothed_sg_150 <- sgolayfilt(a[,2], p = 3, n = 601)
smoothed_sg_15 <- sgolayfilt(a[,3], p = 3, n = 601)
smoothed_sg_3<- sgolayfilt(a[,4], p = 3, n = 601)

plot(smoothed_sg_150[1:14400], type = "l",ylim=c(0,50), col = "black", lwd = 1.5,
     main = "smoothed", ylab = "number", xlab = "time")
lines(smoothed_sg_15[1:14400],col="blue")
lines(smoothed_sg_3[1:14400],col="red")



################################
##### variance
(((1/50)^2)*c^2/((c+1)^2-4*c*A1*3600/50))*((A1*3600/15)^2)*(150/(10)^2)#9.112461
(((1/50)^2)*c^2/((c+1)^2-4*c*A1*3600/50))*((A1*3600/15)^2)*(15/(1)^2) #91.12461
(((1/50)^2)*c^2/((c+1)^2-4*c*A1*3600/50))*((A1*3600/15)^2)*(3/(0.2)^2) #455.6231

(((1/50)^2)*c^2/((c+1)^2-4*c*A1*3600/50))*((A1*3600/15)^2)*(150/(10)^2)-(((1/50)^4)*c^4)*(((A1*3600/15)^2)*(150/(10)^2))^2/((c+1)^2-4*c*A1*3600/50)^3 #9.01924
(((1/50)^2)*c^2/((c+1)^2-4*c*A1*3600/50))*((A1*3600/15)^2)*(15/(1)^2)-(((1/50)^4)*c^4)*(((A1*3600/15)^2)*(15/(1)^2))^2/((c+1)^2-4*c*A1*3600/50)^3 #81.27459
(((1/50)^2)*c^2/((c+1)^2-4*c*A1*3600/50))*((A1*3600/15)^2)*(3/(0.2)^2)-(((1/50)^4)*c^4)*(((A1*3600/15)^2)*(3/(0.2)^2))^2/((c+1)^2-4*c*A1*3600/50)^3 #208.1994

b5_150_80<-(((1/80)^2)*c^2/((c+1)^2-4*c*A1*3600/80))*((A1*3600/15)^2)*(150/(10)^2) #1.221446
b5_15_80<-(((1/80)^2)*c^2/((c+1)^2-4*c*A1*3600/80))*((A1*3600/15)^2)*(15/(1)^2) #12.21446
b5_3_80<-(((1/80)^2)*c^2/((c+1)^2-4*c*A1*3600/80))*((A1*3600/15)^2)*(3/(0.2)^2) #61.07228

(((1/80)^2)*c^2/((c+1)^2-4*c*A1*3600/80))*((A1*3600/15)^2)*(150/(10)^2)-(((1/80)^4)*c^4)*(((A1*3600/15)^2)*(150/(10)^2))^2/((c+1)^2-4*c*A1*3600/80)^3 # 1.221258
(((1/80)^2)*c^2/((c+1)^2-4*c*A1*3600/50))*((A1*3600/15)^2)*(15/(1)^2)-(((1/80)^4)*c^4)*(((A1*3600/15)^2)*(15/(1)^2))^2/((c+1)^2-4*c*A1*3600/80)^3 #35.55743
(((1/80)^2)*c^2/((c+1)^2-4*c*A1*3600/80))*((A1*3600/15)^2)*(3/(0.2)^2)-(((1/80)^4)*c^4)*(((A1*3600/15)^2)*(3/(0.2)^2))^2/((c+1)^2-4*c*A1*3600/80)^3 #59.56755

#### Expectation
(c+1)/2-target/2
# expectation of target^2-variance of target
(c+1)/2-sqrt(((c+1)^2-4*c*A1*3600/50)-4*b5_150)/2 #21.62251
(c+1)/2-sqrt(((c+1)^2-4*c*A1*3600/50)-4*b5_15)/2 #24.89479
(c+1)/2-sqrt(((c+1)^2-4*c*A1*3600/50)-4*b5_3)/2 #NAN

(c+1)/2-sqrt(((c+1)^2-4*c*A1*3600/80)-4*b5_150_80)/2 #11.08803
(c+1)/2-sqrt(((c+1)^2-4*c*A1*3600/80)-4*b5_15_80)/2 #11.31153
(c+1)/2-sqrt(((c+1)^2-4*c*A1*3600/80)-4*b5_3_80)/2 #12.33063

#### Taylor for expectation 
((c+1)-sqrt((c+1)^2-4*c*A1*3600/50))/2 + (1/50)^2*(c^2)*((c+1)^2-4*c*A1*3600/50)^(-3/2)*((A1*3600/15)^2)*(150/(10)^2) # 21.62325
((c+1)-sqrt((c+1)^2-4*c*A1*3600/50))/2 + (1/50)^2*(c^2)*((c+1)^2-4*c*A1*3600/50)^(-3/2)*((A1*3600/15)^2)*(15/(1)^2) #24.45627
((c+1)-sqrt((c+1)^2-4*c*A1*3600/50))/2 + (1/50)^2*(c^2)*((c+1)^2-4*c*A1*3600/50)^(-3/2)*((A1*3600/15)^2)*(3/(0.2)^2) #37.04749

((c+1)-sqrt((c+1)^2-4*c*A1*3600/80))/2 + (1/80)^2*(c^2)*((c+1)^2-4*c*A1*3600/80)^(-3/2)*((A1*3600/15)^2)*(150/(10)^2) # 11.08802
((c+1)-sqrt((c+1)^2-4*c*A1*3600/80))/2 + (1/80)^2*(c^2)*((c+1)^2-4*c*A1*3600/80)^(-3/2)*((A1*3600/15)^2)*(15/(1)^2) #11.31037
((c+1)-sqrt((c+1)^2-4*c*A1*3600/80))/2 + (1/80)^2*(c^2)*((c+1)^2-4*c*A1*3600/80)^(-3/2)*((A1*3600/15)^2)*(3/(0.2)^2) #12.29859

((c+1)-sqrt((c+1)^2-4*c*A1*3600/55))/2 + (1/55)^2*(c^2)*((c+1)^2-4*c*A1*3600/55)^(-3/2)*((A1*3600/15)^2)*(150/(10)^2) # 21.62325
...
plot(a5_150[1:14399,3],xlab = "time",ylab = "number" #,xlim = c(5000,8500)
     ,main = "#of customers in the store ",col="black",ylim = c(0,50),type = "l")
lines(a5_15[1:14399,2],col="blue")
lines(a5_3[1:14399,3],col="red")
legend(5000,50,c("gamma(3,0.2)","gamma(15,1)","gamma(150,10)"),col = c("red","blue","black")
       ,text.col = c("red","blue","black"),bty = "n",cex = 0.8,pch = 19)
abline(h=m,col="green")


plot(customer_number_in_store_save[1:14399,50],xlab = "time",ylab = "number"
     ,main = "#of customers in the store ",col=60,ylim = c(0,50),type = "l")
plot(a5_150[1:14399,1],xlab = "time",ylab = "number"
     ,main = "#of customers in the store ",col="black",ylim = c(0,50),type = "l")
lines(a5_15[1:14399,20],col="blue")
lines(a5_3[1:14399,2],col="red")
lines(customer_number_in_store_save[1:14399,50],col="red")
legend(0,50,c("gamma(3,0.2)","gamma(15,1)","gamma(150,10)"),col = c("red","blue","black")
       ,text.col = c("red","blue","black"),bty = "n",cex = 0.8,pch = 19)

############### delta=80
a5_150_3_80<-a5_150[,c(4,7)]
write.csv(a5_150_3_80,"a5_150_3_80.csv")

plot(a5_150_3_80[1:14399,1],xlab = "time",ylab = "number" #,xlim = c(5000,8500)
    ,main = "#of customers in the store ",col="black",ylim = c(0,30),type = "l")
lines(a5_150_3_80[1:14399,2],col="red")
legend(0,30,c("gamma(3,0.2)","gamma(150,10)"),col = c("red","black")
      ,text.col = c("red","black"),bty = "n",cex = 1,pch = 19)
legend(6000,30,"delta=80",bty="n",cex=1)
###############
...
a6_15<-average_total_spending_rate
a6_150<-average_total_spending_rate
a6_3<-average_total_spending_rate

plot(x=1:14400,y=a6_15[1:14400,50]/45
     ,col=60,ylim=c(0,0.4),type = "l",
     xlab = "time (seoncds)",ylab = NA, main = "shopping rate for total customer in store")
lines(x=1:14400,y=a6_150[1:14400,50]/50,col="red")
legend(0,0.4,c("gamma(3,0..2)","gamma(150,10)"),col = c("blue","red")
       ,text.col = c("blue","red"),bty = "n",cex = 0.6,pch = 19)
